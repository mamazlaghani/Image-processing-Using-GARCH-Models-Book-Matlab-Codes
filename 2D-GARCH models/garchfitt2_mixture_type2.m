function [coefficients,hh,e] = garchfitt2_mixture_type2(spec,y,X);
...
b=0.99999800000000;
A  =  [zeros(1,1+r1+r2+m1+m2+size(X,2)+1)  zeros(1,(p1+1)*(p2+1)-1)  zeros(1,(q1+1)*(q2+1)-1) zeros(1,1+r1+r2+m1+m2+size(X,2)) ones(1,(p1+1)*(p2+1)-1)  ones(1,(q1+1)*(q2+1)-1) 0];
Aeq  =  [];
beq  =  [];
infinity     =  inf;
hundred      =  100;
...
x0  =  [C ; AR(:) ; MA(:) ; regress(:) ; K ; GARCH(:) ; ARCH(:) ;K; GARCH(:) ; ARCH(:); .9]
size(x0);
%[asd,fgh]=size(x0);
...
size(lowerBounds);
[coefficients , LLF, exitFlag , output , lambda] =  fmincon('garchlfn22_mixture_type2'  , x0 , A  , b, Aeq , beq ,lowerBounds ,upperBounds ,'garchnlc2_mixture', [] ,y , r1 , r2 , m1 , m2, p1 , p2, q1 , q2, X);
[LLF , G , H , e , hh] = garchlfn22_mixture_type2(coefficients , y , r1 , r2 , m1 , m2 , p1 ,p2 ,q1 , q2 , X);
