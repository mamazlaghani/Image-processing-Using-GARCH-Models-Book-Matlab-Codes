function [coefficients,hh,e] = garchfitt2_mixture_type1(spec,y,X);
...
b=2*0.99999800000000;
A  =  [zeros(1,1+r1+r2+m1+m2+size(X,2)+1)  ones(1,(p1+1)*(p2+1)-1)  ones(1,(q1+1)*(q2+1)-1) zeros(1,1+r1+r2+m1+m2+size(X,2)) ones(1,(p1+1)*(p2+1)-1)  ones(1,(q1+1)*(q2+1)-1) 0];
Aeq  =  [];
beq  =  [];
infinity     =  inf;
hundred      =  100;
C;
upperBounds  =  [infinity(ones(2*(1+r1+r2+m1+m2+size(X,2))+1+(p1+1)*(p2+1)-1+(q1+1)*(q2+1)-1+(q1+1)*(q2+1)-1+(p1+1)*(p2+1)-1,1)); 1];
x0  =  [C ; AR(:) ; MA(:) ; regress(:) ; K ; GARCH(:) ; ARCH(:) ;K; GARCH(:) ; ARCH(:); .5]
size(x0);
%[asd,fgh]=size(x0);
%x0=zeros(asd,fgh)+.000001;
%specc=garchset;
lowerBounds  =  [-hundred(ones(1+r1+r2+m1+m2,1)) ; -infinity(ones(size(X,2),1)) ; 1e-10 ; zeros([((p1+1)*(p2+1)-1)+((q1+1)*(q2+1)-1)],1); 1e-10 ; zeros([((p1+1)*(p2+1)-1)+((q1+1)*(q2+1)-1)],1);0];
size(lowerBounds);
[coefficients , LLF, exitFlag , output , lambda] =  fmincon('garchlfn22_mixture_type1'  , x0 , A  , b, Aeq , beq ,lowerBounds ,upperBounds ,[], [] ,y , r1 , r2 , m1 , m2, p1 , p2, q1 , q2, X);
[LLF , G , H , e , hh] = garchlfn22_mixture_type1(coefficients , y , r1 , r2 , m1 , m2 , p1 ,p2 ,q1 , q2 , X);
