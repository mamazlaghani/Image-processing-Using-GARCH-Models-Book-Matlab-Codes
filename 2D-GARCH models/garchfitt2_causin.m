function [coefficients,hh,e] = garchfitt2_cousin(spec,y, cc1, cc2, X);
...
b=0.99999800000000;
A  =  [zeros(1,1+r1+r2+m1+m2+size(X,2)+1+2)  ones(1,(p1+1)*(p2+1)-1)  ones(1,(q1+1)*(q2+1)-1)];
Aeq  =  [];
beq  =  [];
infinity     =  inf;
hundred      =  100;
C;
upperBounds  =  [];
x0  =  [1;1 ; C ; AR(:) ; MA(:) ; regress(:) ; K ; GARCH(:) ; ARCH(:)];
size(x0);
%[asd,fgh]=size(x0);
...
size(lowerBounds);
[coefficients , LLF, exitFlag , output , lambda] =  fmincon('garchlfn22_causin'  , x0 , A  , b, Aeq , beq ,lowerBounds ,upperBounds ,[], [] ,y , r1 , r2 , m1 , m2, p1 , p2, q1 , q2, X, cc1, cc2);
[LLF , G , H , e , hh] = garchlfn22_causin(coefficients , y , r1 , r2 , m1 , m2 , p1 ,p2 ,q1 , q2 , X, cc1, cc2);
