...
C;
upperBounds  =  [];
x0  =  [C ; AR(:) ; MA(:) ; regress(:) ; K ; GARCH(:) ; ARCH(:);.4;.6];
size(x0);
%[asd,fgh]=size(x0);
%x0=zeros(asd,fgh)+.000001;
%specc=garchset;
% van=v;
lowerBounds  =  [-hundred(ones(1+r1+r2+m1+m2,1)) ; -infinity(ones(size(X,2),1)) ; 1e-10 ; zeros([((p1+1)*(p2+1)-1)+((q1+1)*(q2+1)-1)],1);.001;.5];
size(lowerBounds);
[coefficients , LLF, exitFlag , output , lambda] =  fmincon('garchlfn22_GG_nonoise'  , x0 , A  , b, Aeq , beq ,lowerBounds ,upperBounds ,[], [] ,y , r1 , r2 , m1 , m2, p1 , p2, q1 , q2,  X);
[LLF , G , H , e , hh] = garchlfn22_GG_nonoise(coefficients , y , r1 , r2 , m1 , m2 , p1 ,p2 ,q1 , q2 , X);
