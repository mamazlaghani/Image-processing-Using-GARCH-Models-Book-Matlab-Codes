function [LLF , G , H , e , hh, hh1 , hh2] = garchlfn22_mixture(Parameters , y , r1 , r2 , m1 , m2 , p1 ,p2 ,q1 , q2 , X);
...
mmm=Parameters(1);
[h,h1,h2]    =  garchlfgarch2_mixture(p1 , p2 ,q1, q2 , nPaths, varianceCoefficients1 , varianceCoefficients2, e2 , T1 , T2, prob, yy, mmm);
...
%         LF=LF*([(1/((h1(i,j))^.5)*exp(-e2(i,j)/(2*h1(i,j))))]);
         ...
         LLF=LLF-log([prob*[(1/((2*pi*h1(i,j))^.5)*exp(-e2(i,j)/(2*h1(i,j))))]+(1-prob)*[(1/((2*pi*h2(i,j))^.5)*exp(-e2(i,j)/(2*h2(i,j))))]]+.00001);
