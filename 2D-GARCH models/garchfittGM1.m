function [coefficients,hh,e] = garchfittGM1(spec,y,pp,X,prob);
R=spec(1);
M=spec(2);
P=spec(3);
Q=spec(4);
...
lowerBounds  =  [-hundred(ones(1+R+M,1)) ; -infinity(ones(size(X,2),1)) ; serr];
...
x0  =  [C ; AR(:) ; MA(:) ; regress(:) ; initialr];
coefficients  =  fmincon('garchlfnGM1'  , x0 , A  , b, Aeq , beq ,lowerBounds ,upperBounds ,@confun22,[] ,y , pp , R , M , P , Q , X, prob);
...
[LLF , G , H , e , hh] = garchlfnGM1(coefficients , y , pp , R , M , P, Q , X, prob);
