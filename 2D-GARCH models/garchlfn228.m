function [LLF , G , H , e , hh] = garchlfn228(Parameters , y , r1 , r2 , m1 , m2 , X);
...
nPaths=1;
 ...
 for i=2*maxpq1+1+r1:T1+2*maxpq1
     for j=2*maxpq2+1+r2:T2+2*maxpq2
         e2(i,j)=e(i-2*maxpq1,j-2*maxpq2).^2;
     end
 end
[T1,T2]  =  size(e2);
varianceCoefficients  =  Parameters((2 + r1 + r2 + m1 + m2 + size(X,2)):end)';
varianceCoefficients(varianceCoefficients <= 0) = realmin;
h    =  garchlfgarch28(varianceCoefficients , e2 , T1 , T2);
LLF=0;
for i=maxpq1+1:T1
    for j=maxpq2+1:T2
