function [coefficients]=garchfit_GG(spec , y , X)
switch nargin
   case 1
     if isnumeric(spec)
        y    =  spec;
...
x0  =  [C ; AR(:) ; MA(:) ; regress(:) ; K ; GARCH(:) ; ARCH(:);0.4,;.6];   % Initial guess.
%
...
infinity     =  inf;
      hundred      =  100;
      lowerBounds  =  [-hundred(ones(1+R+M,1)) ; -infinity(ones(size(X,2),1)) ; 1e-10 ; zeros(P+Q+2,1)];
      upperBounds  =  [];
...
%
[coefficients , LLF, exitFlag , output , lambda] =  fmincon('garchllfn_GG'  , x0 , A  , b, Aeq , beq ,lowerBounds ,upperBounds ,'garchnlc'   , spec.Optimization,y , R , M , P , Q , X);
