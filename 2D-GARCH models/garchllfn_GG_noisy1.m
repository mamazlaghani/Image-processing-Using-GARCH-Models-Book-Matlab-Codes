function [LLF , G , H , e , h] = garchllfn_GG_noisy1(Parameters , y , R , M , P , Q ,van, X)
...
LLF=0;
for i=maxPQ+1:T
    ggg=(quad('object_func',-1000,1000,[],[],e(i,:),van,h(i,:),s,p));
    ggg(ggg<=0) = realmin;
    LLF=LLF+log(ggg);
end
LLF=-LLF;
%LLF=-sum(exp((quad('object_func',-1000,1000,[],[],e(t,:),van,h(t,:),s,p))));
%LLF  = -( LF_fun_GG(e(t,:),van,h(t,:),s,p));
